...
const SIT = [
  { v: "conforme", l: "Conforme", c: "#2E8B57" },
  { v: "nao_conforme", l: "Não conf.", c: "#C0392B" },
  { v: "na", l: "N/A", c: "#6E5C82" },
];
...
function Body() {
  ...
  async function abrir(e: React.FormEvent) {
    ...
    const itens: ItemVistoria[] = CHECKLIST.map((label) => ({ label, situacao: "pendente", obs: "" }));
  ...
  function preencher(v: Vist) {
    ...
    setAberta(v.id);
    const base = (v.itens && v.itens.length ? v.itens : CHECKLIST.map((label) => ({ label, situacao: "pendente", obs: "" })));
    setRascunho({ itens: base.map((i) => ({ ...i, obs: i.obs ?? "" })), observacoes: v.observacoes ?? "" });
  }

  async function salvar(v: Vist) {
    const itens = rascunho.itens;
    const temNC = itens.some((i) => i.situacao === "nao_conforme");
  ...
  return (
    <div className="grid gap-5">
      <p className="rounded-2xl border border-[#f0d3ba] bg-[#fff3e9] p-3 text-[13px] text-[#9a4a12]">
        Abra a vistoria e gere a <b>Ficha Técnica (PDF)</b> numerada para levar a campo. Na volta, lance o checklist:
        os itens <b>não conformes</b> permitem emitir a notificação — ao permissionário (uso) ou à concessionária (estrutura, art. 7º §4º).
      </p>
